...
PASTA_PDFS = "static/pedidos_pdfs"
...
@app.route('/')
def dashboard():
    # 1. DADOS DE CONTAGEM SIMPLES
    total_unidades = Unidade.query.count()
    total_pedidos = Pedido.query.count()
    
    # 2. DADOS PARA O GRÁFICO DE PEDIDOS
    ...
    # 3. DADOS DE COBRANÇA FINANCEIRA
    ...
    # 4. DADOS PARA O GRÁFICO DE DEVEDORES (TOP 5)
    ...
    grafico_nomes = [c.razao_social[:15] + '...' for c in top_devedores] # Corta nome se for mt grande
...
def backup():
    ...
    if os.path.exists(db_file):
        # Gera nome com data (ex: backup_sistema_2025-12-10.db)
...
# ... (MANTENHA AS OUTRAS ROTAS ABAIXO: /tarefas, /raizen, /cobranca, etc.) ...
# Vou incluir as principais para garantir que funcione se você copiar tudo
...
def nova_tarefa():
    ...
    if titulo:
        db.session.add(Tarefa(titulo=titulo, descricao=descricao, prioridade=prioridade, status='todo'))
        db.session.commit()
...
def raizen():
    ...
    else: unidades = Unidade.query.all()
    unidades = sorted(unidades, key=lambda u: any(p.status in ['Pendente', 'Cobrado'] for p in u.pedidos), reverse=True)
...
def marcar_cobrado(id):
    ...
    if p: p.status = 'Cobrado'; db.session.commit(); flash('Marcado como Cobrado.', 'info')
